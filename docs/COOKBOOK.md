...
├── ingest — import_xlsx / import_xls  (xlsx/xls → DB)
├── query (find_header_row_db, iter_rows_by_header)
├── export — writer (DB → xlsx) + merge_policy (충돌 detect/resolve)
...
### Import (`xp_excel_toolkit.ingest`)
...
| `import_xls(session, path, sheet_index=0, header_row=None)` | `.xls` 직접 import (xlrd) |
...
### 변환 / 캐시 (`xp_excel_toolkit.ingest.convert`) + 파이프라인 (`xp_excel_toolkit.pipeline`)
...
| `cache_key(path)` | `(hash, mtime_str)` 튜플 — 캐시 식별자 |
| `ensure_xlsx_cached(path)` | xlsx 면 그대로 반환, xls 면 캐시 디렉토리에 변환 |
| `resolve_db(path, on_progress=None, with_formulas=False, *, import_fn=None)` | xlsx/xls → 캐시된 `.db` path 반환 (없으면 import). `import_fn` 으로 import 함수 교체 가능. `xp_excel_toolkit.pipeline` 소속 |

### Query / Merge 정책 (`xp_excel_toolkit.query` / `xp_excel_toolkit.export.merge_policy`)
...
### Export (`xp_excel_toolkit.export.writer`)
...
- merge 충돌 정책 상세: `xp_excel_toolkit.export.merge_policy` 의 docstring
- 구 모듈 경로 (`helpers`, `convert`, `xlsx_parser`, `xls_parser`, `exporter`) 는
  제거됨 — 새 경로 또는 top-level `xp_excel_toolkit` import 를 사용한다
